...
class KAIST(COCO):
    ...
    def txt2json_my(self,txt,json_file):
        with open(json_file, "r", encoding="utf-8") as f:
            data = json.load(f)   # ✅ 读成字典，而不是文件对象

        # 建立 im_name -> id 的映射
        imname2id = {img["im_name"]: img["id"] for img in data["images"]}
        # print(imname2id)
        ...
        for line in lines:
            ...
            # 取文件名并找到 image_id
            im_name = os.path.basename(img_path).split('.')[0].replace('_','/')
            if im_name not in imname2id:
                raise KeyError(f"{im_name} not found in annotation file!")
            image_id = imname2id[im_name]
    ...
    def loadRes(self, resFile,jsonFile):
        """
        ...
        if type(resFile) == str and resFile.endswith('.txt'):
            # anns = self.txt2json_my(resFile,jsonFile)
...
def evaluate(test_annotation_file: str, user_submission_file: str, phase_codename: str = 'Multispectral', plot=False):
    ...
    kaistDt = kaistGt.loadRes(user_submission_file,test_annotation_file)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--annFile', type=str, default='/home/bq/data/Multi_Spectral_Object_Detection/evaluation_script/KAIST_annotation.json',
                        help='Please put the path of the annotation file. Only support json format.')
    parser.add_argument('--rstFiles', type=str, nargs='+', default='/home/bq/data/datasets/KAIST_7601/KAIST_7601/results/KAIST/Main/comp4_det_test_person.txt',
    ...
    # args.rstFiles = '/home/bq/data/Multi_Spectral_Object_Detection/evaluation_script/KAIST_annotation.json'
    phase = "Multispectral"
    print(args.rstFiles,type(args.rstFiles))
