...
from utils.datasets import create_dataloader_rgb_ir,mask_0_1,create_mask
...
def test(data,
    ...
    h_resolution = int((512 - 16) // 16 + 1)
    w_resolution = int((672 - 16) // 16 + 1)
    clip_model = load_clip_to_cpu('RN50', h_resolution=h_resolution, w_resolution=w_resolution, vision_stride_size=16)
    clip_model.to("cuda")
    image_encoder1 = nn.Sequential(clip_model.visual.conv1, clip_model.visual.bn1, clip_model.visual.conv2,
                                                clip_model.visual.bn2, clip_model.visual.conv3, clip_model.visual.bn3,
                                                clip_model.visual.relu, clip_model.visual.avgpool)

    image_encoder = nn.Sequential(clip_model.visual.layer1, clip_model.visual.layer2, clip_model.visual.layer3,
                                            clip_model.visual.layer4)
    
    attnpool = clip_model.visual.attnpool
    # Half
    ...
    if not training:
        ...
        val_path_nir = data['val_nir']
        dataloader,dataset = create_dataloader_rgb_ir(val_path_rgb, val_path_ir, val_path_nir,imgsz, batch_size, gs, opt, hyp=hyp,augment=False,pad=0.5, rect=True, prefix=colorstr(f'{task}: '))
    ...
    if nc == 1:
        s = ('%20s' + '%12s' * 11) % ('Class', 'Images', 'Labels', 'TP', 'FP', 'FN', 'F1', 'P', 'R', 'mAP@.5',  'mAP@.75','mAP@.5:.95')  # 设置进度条的显示信息
    else:
    ...
    uar = UncertainRouter().to(device)
    uar.load_state_dict(torch.load("/home/bq/data/Multi_Spectral_Object_Detection/weights/router_final.pth", map_location=device))
    uar.eval()  

    for batch_i, (img, targets, absences, paths, shapes) in enumerate(tqdm(dataloader, desc=s)):
        ...
        with torch.no_grad():
            ...
            # 使用CLIP抽取特征
            image_features_map_rgb = image_encoder1(img_rgb)
            image_features_map_rgb = image_encoder(image_features_map_rgb)
            image_proj_rgb = attnpool(image_features_map_rgb)[0]  # [batch_size, embed_dim]

            image_features_map_ir = image_encoder1(img_ir)
            image_features_map_ir = image_encoder(image_features_map_ir)
            image_proj_ir = attnpool(image_features_map_ir)[0]  # [batch_size, embed_dim]

            image_features_map_nir = image_encoder1(img_nir)
            image_features_map_nir = image_encoder(image_features_map_nir)
            image_proj_nir = attnpool(image_features_map_nir)[0]  # [batch_size, embed_dim]
            
            img_rgb = img_rgb.half()
            img_ir = img_ir.half()
            img_nir = img_nir.half()
            image_features_map_rgb = image_features_map_rgb.half()
            image_features_map_ir = image_features_map_ir.half()
            image_features_map_nir = image_features_map_nir.half()
            image_proj_rgb = image_proj_rgb.half()
            image_proj_ir = image_proj_ir.half()
            image_proj_nir = image_proj_nir.half()
            
            # Run model
            t = time_synchronized()
            # print("=====augment:",augment)
            # img_ir = None
            out,_,train_out= model(img_rgb, img_ir,img_nir,absences,mask_tensor_01,clip_feature=(image_features_map_rgb,image_features_map_ir,image_features_map_nir),clip_proj=(image_proj_rgb,image_proj_ir,image_proj_nir),is_training=False)  # inference and training outputs
    ...
    if save_txt:
        temp = []
        # matching_filenames = dataset.get_matching_filenames()
        # print("matching_filenames:",matching_filenames)
        ...
        with open(labels_dir / 'result.txt', 'a') as ff:
            for ii in temp:
                ff.write(ii)
        # with open(labels_dir /'validation_cvc.txt', 'w') as f:
        #     for filename in matching_filenames:
        #         f.write(filename + '\n')

    # 计算MR指标
    # annFile = './evaluation_script/coco_annotations_drnt.json'
    ...
    if nc > 1:
        ...
        pf = '%20s' + '%12i' * 2 + '%12.4g' * 9  # print format
        logger.info(pf % ('all', seen, nt.sum(), tp, fp, fn, f1, mp, mr, map50, map75, map))
    ...
    if not isinstance(tp, int):
        # MRresult
        return (tp[0], fp[0], fn[0], f1[0], mp, mr, map50, map, *(loss.cpu() / len(dataloader)).tolist()), maps, t
    else:
        # MRresult
...
if __name__ == '__main__':
    ...
    parser.add_argument('--weights', nargs='+', type=str, default='/home/bq/data/Multi_Spectral_Object_Detection/runs/train/exp6/weights/best.pt', help='model.pt path(s)')
    parser.add_argument('--data', type=str, default='./data/msod.yaml', help='*.data path')
    ...
    parser.add_argument('--patch', type=bool, default=False, help='patch over images')
    ...
    p = "/home/bq/data/datasets/DRNT_Our/label/test"
    labels_list = os.listdir(p)
    labels_list.sort()
    if opt.data in ['./data/multispectral/FLIR-align-3class.yaml', './data/multispectral/FLIR-ADAS.yaml', './data/multispectral/VEDAI.yaml','./data/multispectral/kaist.yaml']:
        opt.verbose = True
