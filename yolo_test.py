...
class Model(nn.Module):
    ...
    def forward(self, x, x2, x3,absences,mask_tensor_01,clip_feature,clip_proj,augment=False, profile=False,is_training=False):
        if augment:
            ...
            return self.forward_once(x, x2,x3,absences,mask_tensor_01,clip_feature,clip_proj,is_training=is_training)  # single-scale inference, train


    def forward_once(self, rgb, tir, nir,absences,mask_tensor_01,clip_feature,clip_proj,profile=False,is_training=False):
        y, dt = [], []  # outputs
        feature_loss = 0
        absent_rgb, absent_tir, absent_nir = absences[:,0], absences[:,1], absences[:,2]
        x = None
        if clip_feature is not None and clip_proj is not None:
            image_features_map_rgb,image_features_map_ir,image_features_map_nir = clip_feature
            image_proj_rgb,image_proj_ir,image_proj_nir = clip_proj
            # print(image_features_map_rgb.shape,image_features_map_ir.shape,image_features_map_nir.shape)
        else:
            image_features_map_rgb,image_features_map_ir,image_features_map_nir = None,None,None
        ...
        for m in self.model:
            if 0<=m.i <=12:
                if "IntraMA" in m.type:
                    ...
                    if tir is not None:
                        tir,is_absent_tir,loss_tir = m(tir,absent_tir,mask_tensor_01,image_features_map_ir,image_proj_ir)
                    if rgb is not None:
                        rgb,is_absent_rgb,loss_rgb= m(rgb,absent_rgb,mask_tensor_01,image_features_map_rgb,image_proj_rgb)  # run
                    x = None
    ...
    def return_loss(self):
        # print(self.f_loss)
        f_loss =[]
        for fl in self.f_loss:
            f_loss.append(fl.clone().detach())

        # print(sum(f_loss),sum(f_loss).requires_grad)
        return sum(f_loss)
